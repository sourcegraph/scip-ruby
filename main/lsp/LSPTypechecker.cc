...
vector<core::FileRef> LSPTypechecker::runFastPath(LSPFileUpdates &updates, WorkerPool &workers,
    ...
    vector<core::NameHash> changedMethodHashes;
    vector<core::NameHash> changedFieldHashes;
    ...
    gs->errorQueue = make_shared<core::ErrorQueue>(gs->errorQueue->logger, gs->errorQueue->tracer, errorFlusher);
    {
        ...
        for (auto &f : updates.updatedFiles) {
            ...
            if (fref.exists()) {
                ...
                const auto &newSymbolHashes = f->getFileHash()->localSymbolTableHashes;
        ...
        core::NameHash::sortAndDedupe(changedMethodHashes);
        ...
        absl::c_transform(changedFieldSymbolHashes, std::back_inserter(changedFieldHashes),
                          [](const auto &symhash) { return symhash.nameHash; });
        core::NameHash::sortAndDedupe(changedFieldHashes);
    }
