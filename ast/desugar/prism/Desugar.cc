...
class Desugarer final {
    ...
    // Unique counter used to create synthetic names via `ctx.state.freshNameUnique`,
    // just like the `DesugarContext.uniqueCounter`  in `ast/desugar/Desugar.cc`
    //
    // This storage integer either stores an "active" count used by a translator and some of its children,
    // or a dummy value.
    uint32_t uniqueCounterStorage;
    // Points to the "active" count for this translator, which either points to its own storage,
    // or to a parent's storage.
...
public:
    Desugarer(const Parser &parser, core::MutableContext ctx, bool preserveConcreteSyntax,
              core::LocOffsets &enclosingBlockParamLoc, core::NameRef &enclosingBlockParamName)
        : parser(parser), ctx(ctx), preserveConcreteSyntax(preserveConcreteSyntax), uniqueCounterStorage(1),
          uniqueCounter(this->uniqueCounterStorage), enclosingBlockParamLoc(enclosingBlockParamLoc),
...
private:
    ...
    Desugarer(const Desugarer &parent, bool resetUniqueCounter, core::NameRef enclosingMethodName,
              core::LocOffsets &enclosingBlockParamLoc, core::NameRef &enclosingBlockParamName, bool isInModule,
              bool isInAnyBlock)
        : parser(parent.parser), ctx(parent.ctx), preserveConcreteSyntax(parent.preserveConcreteSyntax),
          uniqueCounterStorage(resetUniqueCounter ? 1 : 999999),
    ...
    ast::ExpressionPtr desugarConditionalSend(core::LocOffsets location, pm_node_t *receiver,
...
template <typename Lambda>
ast::ExpressionPtr Desugarer::desugarConditionalSend(core::LocOffsets location, pm_node_t *receiver,
                                                     pm_constant_id_t methodNameID, pm_location_t methodNamePrismLoc,
                                                     Lambda &&body) {
    ENFORCE(receiver != nullptr, "Conditional sends should always have a receiver.");

    auto loc0 = location.copyWithZeroLength();

    // Create the temporary local variable before desugaring the receiver,
    // so that chained csends get numbered from left to right.
    auto receiverTempLocalVarName = nextUniqueName(core::Names::assignTemp());

    auto receiverExpr = desugar(receiver);
    ...
    auto recvLoc = translateLoc(receiver->location);
...
ast::ExpressionPtr Desugarer::desugarSendOpAssign(pm_node_t *untypedNode) {
    ...
    if (PM_NODE_FLAG_P(untypedNode, PM_CALL_NODE_FLAGS_SAFE_NAVIGATION)) {
        ...
        return desugarConditionalSend(lhsLoc, node->receiver, node->read_name, node->message_loc, body);
    }
    ...
    switch (PM_NODE_TYPE(node)) {
        ...
        case PM_CALL_NODE: { // A method call like `a.b()` or `a&.b()`
            ...
            if (PM_NODE_FLAG_P(callNode, PM_CALL_NODE_FLAGS_SAFE_NAVIGATION)) {
                ...
                return desugarConditionalSend(location, callNode->receiver, callNode->name, callNode->message_loc,
                                              body);
            }
        ...
        case PM_FOR_NODE: { // `for x in a; ...; end`
            ...
            if (canProvideNiceDesugar) {
                ...
                core::NameRef forLoopTempVarName = nextUniqueName(core::Names::forTemp());
                ast::MethodDef::PARAMS_store params;
        ...
        case PM_SUPER_NODE: { // A `super` call with explicit args, like `super()`, `super(a, b)`
            ...
            // not just the `super` keyword. This matches the behavior in the legacy Desugar.cc.
...
core::NameRef Desugarer::nextUniqueName(core::NameRef original) {
...
Desugarer Desugarer::enterMethodDef(bool isSingletonMethod, core::NameRef methodName,
                                    core::LocOffsets &enclosingBlockParamLoc,
                                    core::NameRef &enclosingBlockParamName) const {
    auto resetUniqueCounter = true;
    auto isInModule = this->isInModule && !isSingletonMethod;
    return Desugarer(*this, resetUniqueCounter, methodName, enclosingBlockParamLoc, enclosingBlockParamName, isInModule,
                     this->isInAnyBlock);
}

Desugarer Desugarer::enterBlockContext() const {
    auto resetUniqueCounter = false; // Blocks inherit their parent's numbering
    auto isInAnyBlock = true;
    return Desugarer(*this, resetUniqueCounter, this->enclosingMethodName, this->enclosingBlockParamLoc,
...
Desugarer Desugarer::enterModuleContext(core::LocOffsets &enclosingBlockParamLoc,
    ...
    return Desugarer(*this, resetUniqueCounter, this->enclosingMethodName, enclosingBlockParamLoc,
...
ExpressionPtr node2Tree(core::MutableContext ctx, parser::Prism::ParseResult parseResult, bool preserveConcreteSyntax) {
    ...
    auto expr =
        Desugarer(parseResult.getParser(), ctx, preserveConcreteSyntax, enclosingBlockParamLoc, enclosingBlockParamName)
